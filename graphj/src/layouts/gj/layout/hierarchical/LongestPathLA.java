...
public class LongestPathLA implements LayerAssignment {
  ...
  public List<Layer> assignLayers(Graph graph, Layout2D layout) throws GraphNotSupportedException {
    ...
    // prepare state
    Map<Vertex, Layer.Assignment> vertex2assignment = new HashMap<Vertex, Layer.Assignment>();
    List<Layer> layers = new ArrayList<Layer>();

    // find sinks
    for (Vertex v : graph.getVertices()) {
      if (LayoutHelper.isSink(v)) 
        sinkToSource(null, v, new Stack<Layer.Assignment>(), vertex2assignment, layers, layout);
    }
...
//      
//      for (Layer.Assignment sink : layer) {
//        
//        for (int j=0;j<sink.incoming().size();j++) {
//          
//          Node source = sink.incoming().get(j);
//          if (source.layer()!=i+1) {
//
//            // add a dummy vertex/assignment
// FIXME need dummy vertices
//            Node dummy = new Node(source, sink, i+1, layout.getPositionOfVertex(sink.vertex()).getX());
...
//    }
   ...
   */
  private void sinkToSource(Edge edge, Vertex vertex, Stack<Layer.Assignment> path, Map<Vertex, Layer.Assignment> vertex2assignment, List<Layer> layers, Layout2D layout) throws GraphNotSupportedException{
    ...
    Layer.Assignment assignment = vertex2assignment.get(vertex);
    if (assignment==null) {
      assignment = new Layer.Assignment(vertex, -1, layout.getPositionOfVertex(vertex).getX());
