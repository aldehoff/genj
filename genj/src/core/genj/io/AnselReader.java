...
/**
 * Read for decoding ANSEL - this should probably be captured
 * in a java.nio.charset.Charset but that's coming with 1.4.x only
 * 
 * @author mhkay@iclway.co.uk original
 * @author nils@meiers.net adoption
 */
public class AnselReader extends Reader {
   ...
   * Constructor
   */
  public AnselReader(InputStream stream) {
    in = stream;
  }
  ...
  public int read() throws IOException {
    ...
    if (next < 0) {
      close();
      return ansel1byte2unicode(pending);
    }
    // no pending?
    if (pending < 0) {
      pending = next;
    ...
    // try to convert 
    int unicode = ansel2byte2unicode(pending << 8 | next);
    if (unicode>0) {
      pending = -1;
      return unicode;
    }
    ...
    pending = next;
    // done    
    return ansel1byte2unicode(result);
  }
