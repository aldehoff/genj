...
public class ReportPhonetics extends Report {
    ...
    private void printPhonetic(Indi indi) {
        ...
        println(i18n("indiName")+": "+name);
        
        switch(outputFormat) {
            
            case 0:
                // print the Soundex codes
                println("Soundex: "+Soundex.sencode(firstName)+" "+Soundex.sencode(lastName));
                break;
            case 1:
                
                //print the Metaphone codes
                println("Metaphone: "+MetaPhone.metaPhone(firstName)+" "+MetaPhone.metaPhone(lastName));
                break;
                
            case 2:
                //print the Double Metaphone codes
                println("DoubleMetaphone: "+DoubleMetaphone.sencode(firstName)+" "+DoubleMetaphone.sencode(lastName));
                break;
                
            case 3:
                //print the NYSIIS codes
                println("NYSIIS: "+Nysiis.sencode(firstName)+" "+Nysiis.sencode(lastName));
                break;
                
            case 4:
                //print the Phonex codes
                println("Phonex: "+Phonex.sencode(firstName)+" "+Phonex.sencode(lastName));
                break;
        }
        println();
    }
  ...
  static class DoubleMetaphone {
      ...
      public String encode(String in) {
          ...
          // only the first word
          BreakIterator wi = BreakIterator.getWordInstance(java.util.Locale.ENGLISH);
          wi.setText(in);
          int start = wi.first();
          int end = wi.next();
          if(end != BreakIterator.DONE) in = in.substring(start,end);
          ...
          while (primary.length() < encodeLimit || alternate.length() < encodeLimit) {
              ...
              switch(input.charAt(current)) {
                  ...
                  //case '—':   using ASCII code to make it compile with french locale
                  case 165:
                      current++;
                      add('N');
                      break;
  ...
  static class MetaPhone {
      ...
      public static String metaPhone(String txt){
          ...
          int start = wi.first();
          int end = wi.next();
          if(end != BreakIterator.DONE) txt = txt.substring(start,end);
          
          // check the first letter is a character
          if(!Character.isLetter( txt.charAt(0))) return metaPhone(txt.substring(1));
  ...
  static class Nysiis {
      ...
      public String encode( String originalWord ) {
          if(originalWord == null || originalWord.length() == 0) return null;
          
          // only the first word
          BreakIterator wi = BreakIterator.getWordInstance(java.util.Locale.ENGLISH);
          wi.setText(originalWord);
          int start = wi.first();
          int end = wi.next();
          if(end != BreakIterator.DONE) originalWord = originalWord.substring(start,end);
  ...
  static class Phonex {
      ...
      static public final char[] CHAR_MAPPING =
      ...
      public String phonex( String txt ){
          ...
          BreakIterator wi =
          BreakIterator.getWordInstance(java.util.Locale.ENGLISH);
          ...
          while( (incount < processedString.length() ) &&
          (mapped = getCode(processedString, incount++)) != 0 &&
          (count < maxCodeLen) ) {
          ...
          while(code.length() < maxCodeLen) code.append('0');
      ...
      private char getCode(Character prev, Character c, Character next) {
          if( c == null || !Character.isLetter(c.charValue()) ) {
              ...
              if((c.charValue() == 'D' || c.charValue() == 'T')
              && (next != null && next.charValue() == 'C'))
                  return '0';
              // L or R followed by vowel or end of name
              else if((c.charValue() == 'L' || c.charValue() == 'R') &&
              (next == null || next.charValue() == 'A' ||
              next.charValue() == 'E' || next.charValue() == 'I' ||
              next.charValue() == 'O' || next.charValue() == 'U'))
                  return '0';
              // D or G preceded by M or N
              else if((c.charValue() == 'D' || c.charValue() == 'G') &&
              (prev != null && (prev.charValue() == 'M'
              || prev.charValue() == 'N')))
                  return '0';
              else {
                  int loc = Character.toUpperCase(c.charValue()) - 'A';
                  if (loc < 0 || loc > (CHAR_MAPPING.length - 1)) return '0';
  ...
  static class Soundex {
      ...
      static public final char[] US_ENGLISH_SOUNDEX_MAPPING =
      "01230120022455012623010202".toCharArray();
       ...
       */
      private static String replaceAll(String s, String ss, String rs) {
          int i = s.indexOf(ss);
          if (i == -1) return s;
          int ssSize = ss.length();
          
          String part1 = s.substring(0, i);
          String part2 = replaceAll((s.substring(i + ssSize)),ss,rs);
          s = part1 + rs + part2;
          return s;
      }
       ...
       * FIXME this is slow!
       */
      private static String substituteAccents(String str){
          String result;
          result = replaceAll(str, "ﬁ", "TH");
          result = replaceAll(result, "˛", "th");
          result = replaceAll(result, "–", "DH");
          result = replaceAll(result, "", "dh");
          result = replaceAll(result, "ﬂ", "ss");
          result = replaceAll(result, "å", "OE");
          result = replaceAll(result, "ú", "oe");
          result = replaceAll(result, "∆", "AE");
          result = replaceAll(result, "Ê", "ae");
          char[] accents =    "äéöûü¿¡¬√ƒ≈«»… ÀÃÕŒœ—“”‘’÷ÿŸ⁄€‹›‡·‚„‰ÂÁËÈÍÎÏÌÓÔÒÚÛÙıˆ¯˘˙˚¸˝µ".toCharArray();
          char[] nonAccents = "SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyu".toCharArray();
          for(int i=0;i<accents.length;i++) result = result.replace(accents[i],nonAccents[i]);
          return result;
      }    
      ...
      public String soundex(String s) {
          ...
          int start = wi.first();
          int end = wi.next();
          if(end != BreakIterator.DONE) s = s.substring(start,end);
          
          // FIXME should handle accents better!
          // should get a true code for each acented character
          String str = substituteAccents(s);
          
          // check the first letter is a character
          if(!Character.isLetter( str.charAt(0))) return soundex(str.substring(1));
      ...
      private char getMappingCode(char c) {
          if( !Character.isLetter(c) ) {
              ...
              if (loc < 0 || loc > (soundexMapping.length - 1)) return '0';
